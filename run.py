...
from arch.utils import device_mapper, load_config, chat_playback, albedo
...
def main():
    ...
    dialogs: List[Dialog] = [[{"role": "system", "content": system_call}]]
    ...
    while True:
        ...
        logprobs = False
        new_toks, _ = LLM.generate(prompt_tokens=toks, max_gen_len=None, temperature=0.6, top_p=0.9, logprobs=logprobs)

        if logprobs:
            result = [{"generation": {"role": "assistant", "content": LLM.tokenizer.decode(t)}, "tokens": [LLM.tokenizer.decode(x) for x in t], "logprobs": logprobs_i,} for t, logprobs_i in zip(new_toks, logprobs)]
        else:
